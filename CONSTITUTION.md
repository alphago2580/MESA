...
- **문서/명세** (`.specify/`, `.claude/`, `.cursor/` 등): 명세서, 설계 문서, 가이드
...
- `.gitignore`에 `.env`, `config.json` 등 시크릿 파일 패턴을 반드시 포함하고, 필요 시 `*.pem` 등 추가 시크릿 파일 패턴을 예시로 활용한다
...
- **프레임워크 표준:** 신규/개편되는 Core Logic 테스트의 표준 러너는 `pytest`로 한다.
  - 기존 `unittest` 기반 테스트는 점진적 마이그레이션 완료 시까지 허용하며, 혼합 기간 동안 두 러너 모두 통과해야 한다.
  - `pytest` 도입 PR에서는 `requirements.txt`에 `pytest`를 추가하고, `README`/CI 설정에 `pytest tests/` 기준의 실행 방법을 명시해야 한다.
  - 기존 `unittest` 테스트는 `python -m unittest`로 실행하며, `pytest` 도입 후에는 가능하면 테스트를 `pytest` 스타일로 변환한다.
...
- 명세서(`.specify/`, `.claude/` 등), README, 가이드 문서는 한국어 작성
...
- **TypeScript (`discord-github-worker/`):** `interface` 정의를 강제하며, `any` 타입을 절대 사용하지 않는다.
- **Linter:** `ruff` 및 `mypy` 검사를 통과하도록 코드를 작성하는 것을 권장한다. (향후 CI에서 자동 검사로 강제 예정)
